...
class MetaNeXtStage_up(nn.Module):
    def __init__(self, in_chs, out_chs, kernel_size, padding, depth=2, drop_path_rates=None, ls_init_value=1e-6, token_mixer=InceptionDWConv2d, act_layer=nn.GELU, norm_layer=nn.BatchNorm2d, mlp_ratio=4):
        super().__init__()
        self.grad_checkpointing = False
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv = nn.Conv2d(in_chs, out_chs, kernel_size, stride=1, padding=padding, dilation=1, bias=False)
        self.bn = nn.BatchNorm2d(out_chs)
        dp_rates = 0.1
        drop_path_rates= torch.linspace(0, dp_rates, depth).tolist()
...
def inceptionnext_tiny(pretrained=False, **kwargs):
    ...
    pth_path = r'D:\pythyon_work\pythonProject1\MDI-Net\pretrained\inceptionnext_tiny.pth'
    state_dict = torch.load(pth_path, map_location='cpu')
    model.load_state_dict(state_dict, strict=False)
    print('Pretrained inceptionnext Model Have been loaded!')
    return model
